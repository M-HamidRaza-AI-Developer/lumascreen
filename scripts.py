...
function initSliders() {
  // 1. Movies Slider Logic
  document.querySelectorAll('.slider-container').forEach(container => {
    // Top quick bar ko yahan se ignore karna hai
    ...
    if (slider && leftArrow) {
      const updateMovieArrow = () => {
        leftArrow.style.display = slider.scrollLeft > 10 ? 'flex' : 'none';
      };
      updateMovieArrow();
      slider.removeEventListener('scroll', updateMovieArrow);
      slider.addEventListener('scroll', updateMovieArrow);
    }
  });

  // 2. Top Quick Bar Logic
  const quickBar = document.querySelector('.top-quick-bar');
  const quickArrow = document.querySelector('.back-arrow-btn');
  if (quickBar && quickArrow) {
    const updateQuickArrow = () => {
      quickArrow.style.display = quickBar.scrollLeft > 10 ? 'flex' : 'none';
    };
    updateQuickArrow();
    quickBar.removeEventListener('scroll', updateQuickArrow);
    quickBar.addEventListener('scroll', updateQuickArrow);
  }
}
